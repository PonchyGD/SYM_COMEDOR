...
if (!$es_fin_de_semana) {
    ...
    try {
        ...
        $vale_check = $conn->prepare("SELECT vale FROM empleados WHERE nombre = :nombre_empleado");
        $vale_check->bindParam(':nombre_empleado', $nombre_empleado, PDO::PARAM_STR);
        $vale_check->execute();
        $vale = $vale_check->fetch(PDO::FETCH_ASSOC);
...
?>
    <h2 id="menu" class="roboto-black">Hola, <?php echo $nombre_empleado; ?>.</p>
    <h2 id="menu">Elija su platillo de hoy: <br> <?php echo date("d"); ?> de octubre.</h2>
    ...
    <div class="tab-container">
        ...
        <?php if ($vale['vale'] && $vale = 1) { ?> 
                <button class="tab" onclick="openTab(event, 'cupon')">
                    <span class='roboto-black'>Vale Kabat:</span>
                </button> 
            <?php } else { ?>
